...
func firstLetter(text string) (string, error) {
    if len(text) < 1 {
        return "", errors.New("Parameter text is empty")
    }
...
// Channels can be bufferized. Buffered channels will accept a limited number of values and when someone try to put beyond their limit, it will throw an error
